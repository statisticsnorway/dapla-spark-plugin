...
package no.ssb.dapla.gcs.token.issuer;
...
public class SignedJWTProvider {

    private boolean brokerIssuer;

    public SignedJWTProvider(boolean brokerIssuer) {
    ...
    private AccessTokenProvider.AccessToken tradeSignedJWTForAccessToken(String signedJWT) {
        HttpTransport httpTransport;
        JsonFactory jsonFactory;
        ...
        return new AccessTokenProvider.AccessToken(
            response.getAccessToken(),
            System.currentTimeMillis() + response.getExpiresInSeconds() * 1000);
    }

    public AccessTokenProvider.AccessToken getAccessToken(String owner, String scope) {
        // Get signed JWT
